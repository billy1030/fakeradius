#!/bin/bash
# Test PAP authentication with a "no_" prefixed user (should get Access-Reject)
# Usage: ./test-pap-no-user.sh [username] [secret] [server]
...
"${BIN_DIR}/radius-cli" --username "$USERNAME" --password "testpass123" --secret "$SECRET" --server "$SERVER"
